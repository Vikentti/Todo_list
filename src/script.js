class Todo {
  ...
  constructor() {
    ...
    this.emptyMessageElement = this.rootElement.querySelector(this.selectors.emptyMassage)
    this.state = {
  ...
  getItemsFromLocalStorage() {
    ...
    try {
      ...
      console.log(new Error`Ошибка в локал сторедже`)
    }
  }
  ...
  filter() {
    const formatedQuery = this.state.searchQuery.toLowerCase()

    this.state.filteredItems = this.state.items.filter(({title}) => {
      const titleFormatted = title.toLowerCase()

      return titleFormatted.includes(formatedQuery)
    })
    this.render()
  }
  ...
  onChange = ({ target }) => {
    if (target.matches(this.selectors.itemCheckbox)) {
      this.toggledCheckedState(target.id)
    }
  }
  ...
  bindEvents() {
    ...
    this.listElement.addEventListener('click', this.onClick, { capture: true })
